...
export default function LeadModal({ isOpen, onClose, type }) {
  ...
  // ✅ Address field changes ko handle karega
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ 3. Proper payload structure for both types
    const payload = {
      ownerName: formData.ownerName,
      leadType: type,
      email: formData.email,
    ...
    // ✅ 4. Add type-specific fields with complete address for Salon
    if (type === "Salon") {
      payload.salonName = formData.salonName;
      payload.address = {
        street: formData.address.street,
        area: formData.address.area || "", // Agar area empty hai to empty string send karein
        city: formData.address.city,
        state: formData.address.state || "Rajasthan", // Default state agar empty hai
    ...
    try {
      const res = await axiosInstance.post("/leads", payload); 
      console.log("Response:", res); // Debug ke liye
      ...
      console.error("Error:", err.response?.data || err); // Detailed error
      const errorMsg = err.response?.data?.message || "Something went wrong!";
      Swal.fire({
        title: 'Error!',
        text: errorMsg,
